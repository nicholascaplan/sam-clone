...
- [x] **Review the homepage content mix.** Removed Events and past performances from the homepage while retaining the event records in the content source and specification.
...
- Event records remain documented newest first; the homepage calendar has since been removed.
...
- [x] **Fix mobile back navigation.** Section and Works & Media view changes now create browser-history entries, with `popstate` restoring the prior in-site view before Back can leave the site. Verify on supported mobile browsers before deployment.
...
- Session note, 2026-08-01: The shared Listen catalogue now shows `30 sec preview` in the top metadata line for Spotify recordings, replacing the earlier Extract treatment. The SoundBar uses a fixed 30-second duration for every Spotify preview. SoundCloud durations remain visible. Live third-party playback still needs manual verification outside the BoxedCode sandbox.
...
- Session note, 2026-08-02: Implemented the selected Playground Option A in the live shared Listen renderer. Spotify recordings now show `30 sec preview` after `Recording` in the top metadata line without displaying the provider name. The legacy source rows remain in `index.html` for playback and metadata source compatibility; the rendered `#listen` view uses `renderMediaCatalog`.
...
- Session note, 2026-08-02: The Playground's retired `Extract` row comparison was replaced with the current live Listen card as a baseline and three 30-second preview-label treatments using the same card structure. Option A adds `30 sec preview` to the existing year and Recording metadata line and is now applied to the shared live catalogue; the provider remains hidden.
...
- Session note, 2026-08-02: Fixed the Watch catalogue card stagger caused by `space-y-4` adding margins to dynamically rendered article cards. The shared catalogue now uses an explicit grid gap, with two columns from `768px` and three Watch columns from `1280px`; Works and Listen remain two columns. Static checks pass; browser verification remains blocked by the BoxedCode sandbox's Chromium macOS rendezvous permission error.
- Session note, 2026-08-02: Removed the redundant `Film` label from Watch card metadata because the Watch view already establishes that every item is a film. The year remains as the card's metadata label.
...
- Session note, 2026-08-02: Session closed after implementing Option A for Spotify previews. `git diff --check`, JavaScript syntax checks and Playwright test discovery should be run before deployment; the targeted browser tests were attempted but remain blocked by the BoxedCode sandbox's macOS Chromium rendezvous permission error. Manually verify the `30 sec preview` metadata and 30-second SoundBar in an unrestricted browser or CI run.
- Session note, 2026-08-02: The first GitHub Pages deployment attempt passed 30 of 31 smoke tests but failed because the new preview assertion targeted the nested Listen button instead of its enclosing card. The assertion was corrected to inspect the card metadata; rerun the Pages workflow after the follow-up push.
...
- Session note, 2026-08-02: Replaced section and Works & Media view `replaceState` calls with pushed history entries and added `popstate` rendering, so browser Back and Forward restore in-site views. Static JavaScript, whitespace and Playwright discovery pass. Targeted browser tests remain blocked before assertions by the sandbox's Chromium macOS `MachPortRendezvousServer` permission error; verify Back and Forward on desktop and mobile in an unrestricted browser before deployment.
- Session note, 2026-08-02: Works, Listen and Watch now reset the shared search and Instrumentation filters when the selected view changes. Added Playwright coverage for the reset across Watch and Works transitions; run the targeted browser test in an unrestricted environment before deployment.
- Session note, 2026-08-02: Catalogue card text now top-aligns across grid items while media actions remain bottom-positioned on desktop. `git diff --check` passes; browser verification remains blocked by the BoxedCode sandbox's Chromium macOS rendezvous permission error.
- Session note, 2026-08-02: Biography is an in-page `#bio` route in `index.html`; the root route retains the homepage quote, portrait, destination actions, short introduction and Spotlight Works. Do not add standalone public HTML pages until a documented architecture split is approved. Static checks and test discovery pass. The focused Biography and browser-history tests were attempted but could not launch Chromium because the BoxedCode sandbox denies macOS `MachPortRendezvousServer` registration. Run `npm test` in an unrestricted terminal or CI before deployment.
- Session note, 2026-08-02: The first deployment attempt passed 34 smoke tests but caught an accessibility contrast regression in light-mode Works view controls caused by inline colour styles overriding the shared light-mode CSS. Removed those inline styles so the explicit accessible CSS states control the buttons; rerun the Pages workflow.
