...
- Session note, 2026-08-01: All Spotify Listen rows now show a quiet `Extract` status in place of duration because Spotify playback uses 30-second previews. The SoundBar now also uses a fixed 30-second duration for every Spotify preview. SoundCloud durations remain visible. Live third-party playback still needs manual verification outside the BoxedCode sandbox.
...
- Session note, 2026-08-02: The Playground's retired `Extract` row comparison was replaced with the current live Listen card as a baseline and three 30-second preview-label treatments using the same card structure. Option A adds `30 sec preview` to the existing year and Recording metadata line and is the recommended direction; the provider remains hidden and no option has been applied to the live catalogue yet.
